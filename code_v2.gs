...
function onOpen() {
  SpreadsheetApp.getUi() // Or DocumentApp or FormApp.
    ...
    .addToUi();
  Browser.msgBox('Instructions - Please Read', '1) Click on Date Range in the menu bar.\\n2) Choose Select Dates.\
...
function processForm(formObject) {
  ...
  function export_gcal_to_gsheet() {
    ...
    var remaining = sheet.getRange("a6");
    remaining.setValue("Remaining:");
    var bold = sheet.getRange(1, 1, 6, 1);
    ...
    var header = [
      ["Description", "Transaction Type", "Amount", "Due Date", "Last Updated", "Paid"]
    ]
    var range = sheet.getRange(8, 1, 1, 6);
    range.setValues(header);
    range.setFontWeight("bold")
    range.setHorizontalAlignment("center");
    range.setBorder(true, true, true, true, null, null);

    // Set the data-validation rule for cells in Paid column starting at cell 'F9' to show a dropdown with Yes/No.
    var cells = SpreadsheetApp.getActive().getRange('F9:F');
    var rule = SpreadsheetApp.newDataValidation().requireValueInList(['Yes', 'No'], true).build();
    cells.setDataValidation(rule);

    // Loop through all calendar events found and write them out starting on calculated ROW 9 (i+9)
    for (var i = 0; i < events.length; i++) {
      ...
      var details = [
        [events[i].getTitle(), events[i].getDescription(), myformula_placeholder, events[i].getStartTime(), events[i].getLastUpdated()]
      ];
      ...
      // Write the formula out for every row in column 3 to match the position of the field myformula_placeholder from above.
    ...
    countDeposits.setFormula("=COUNTA(filter(C9:C,C9:C>0))").setNumberFormat("0").setHorizontalAlignment("left");
    var countWithdrawals = sheet.getRange("b3");
    countWithdrawals.setFormula("=COUNTA(filter(C9:C,C9:C<0))").setNumberFormat("0").setHorizontalAlignment("left");
    var sumIncome = sheet.getRange("b4");
    sumIncome.setFormula("=SUM(filter(C9:C,C9:C>0))").setNumberFormat("$0.00").setHorizontalAlignment("left");
    var sumExpenses = sheet.getRange("b5");
    sumExpenses.setFormula("=SUM(filter(C9:C,C9:C<0))").setNumberFormat("$0.00").setHorizontalAlignment("left").setFontColor("#c53929").setFontWeight("bold");
    var sumRemaining = sheet.getRange("b6");
    sumRemaining.setFormula("=SUM(b4:b5)").setNumberFormat("$0.00").setHorizontalAlignment("left");

    // freeze first eight rows
